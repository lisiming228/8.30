    ...
    function tree($pid=0,&$str,$i=0){
        ...
        while ($row=$result->fetch_assoc()){
            ...
            $str.='<tr><td>'.($i+1).'级目录</td><td>'.str_repeat("-->",$i).$row["cname"].'</td><td>'.$s.'</td><td>
                ...
                <a href="/phpStudyTest/mvc2/index.php/admin/category/del?cid='.$row["cid"].'" attr="'.$row["cid"].'" class="remove">删除</a>
...
//        $path=$_POST["imgurl"];
        ...
        $database=new db();
        $db=$database->db;
        $db->query("insert into mvc_category (cname,pid,isshow,tpl_name,info,exp,imgurl) values ('$cname','$cid','$isshow','$tpl_name','$info','$exp','$path')");
        if($db->affected_rows>0){
            header("location:/phpStudyTest/mvc2/index.php/admin/category");
        }
    }
    function del(){
        //删除栏目
        ...
        if($result->num_rows>0){
            echo "<script> alert('请先删除子目录');location.href='/phpStudyTest/mvc2/index.php/admin/category'</script>";
        }else{
